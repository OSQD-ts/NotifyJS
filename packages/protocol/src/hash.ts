 ...
 *
 * The pairing-code check characters must be computed identically on the hub
 * (Node), in the dashboard (browser) and on the phone (React Native). WebCrypto
 * only offers an async digest and React Native has no `node:crypto`, so a
 * ~60-line implementation is the one thing all three can share. It is used for
 * checksums and store lookup keys only — never for signatures.
 */
...
export function sha256Hex(data: Uint8Array): string {
  return [...sha256Sync(data)].map((b) => b.toString(16).padStart(2, '0')).join('');
}
